...
const loadStats = async () => {
    loading.value = true
    try {
        const result = await expedientesStore.fetchStatistics()
        if (result.success && expedientesStore.statistics) {
            ...
            <div class="px-6 lg:px-8">
                <div class="flex justify-between items-center h-16">
                    ...
                    <div class="flex items-center gap-4">
                        <div class="flex items-center gap-3">
                            <Avatar :label="userInitials" shape="circle" class="bg-gray-200 text-gray-700"
                                size="normal" />
                        ...
                        <Button icon="pi pi-sign-out" severity="secondary" text rounded
        ...
        <main class="px-6 lg:px-8 py-8 max-w-7xl mx-auto">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                <div v-for="link in dashboardLinks" :key="link.id" @click="navigateTo(link)"
                    class="group relative rounded-xl p-6 transition-all duration-200 cursor-pointer" :class="[
                        getBgColor(link.color),
                        link.available ? '' : 'opacity-60'
                    ]">
                    <!-- Badge de estado -->
                    ...
                    <div class="absolute bottom-4 right-4">
                        ...
                        <i v-else
                            class="pi pi-arrow-right text-gray-400 group-hover:translate-x-1 transition-transform"></i>
                    </div>
                </div>
            </div>
