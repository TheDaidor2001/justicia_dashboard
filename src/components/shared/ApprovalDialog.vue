...
<template>
    <Dialog :visible="visible" @update:visible="$emit('update:visible', $event)" @hide="handleHide" modal
        :header="dialogHeader" :style="{ width: '500px' }" :closable="!loading">
        ...
        <div class="mb-4">
            ...
            <Textarea v-model="comments" rows="4" class="w-full" :placeholder="isReject
                ? 'Explica el motivo del rechazo...'
                : 'Añade comentarios sobre la aprobación...'"
                :class="{ 'p-invalid': commentsRequired && !comments.trim() }" :disabled="loading"
        ...
        <template #footer>
            <div class="flex gap-2 justify-end">
                ...
                <Button :label="confirmLabel" :icon="loading ? 'pi pi-spinner pi-spin' : confirmIcon"
                    :severity="confirmSeverity" @click="handleConfirm" :loading="loading" :disabled="!isValid" />
            </div>
        </template>
    </Dialog>
</template>
